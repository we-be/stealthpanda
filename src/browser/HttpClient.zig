...
/// The innermost handler: owns the curl multi, connection pool, transfer pool,
/// interception state, auth-challenge retries, and redirect following.
/// Everything that was previously on Client that is *not* robots or cache.
pub const Transport = struct {
    // Count of active requests
    active: usize = 0,

    // Count of intercepted requests. See original comment.
    intercepted: usize = 0,
    ...
    pub fn init(allocator: Allocator, network: *Network) !*Transport {
    ...
    /// Core entry point — does interception gating then queues/starts the
    /// transfer.  Robots and cache checks are done by the layers above.
    ...
    fn processOneMessage(self: *Transport, msg: http.Handles.MultiMessage, transfer: *Transfer) !bool {
        ...
        transfer.req.notification.dispatch(.http_request_done, &.{ .transfer = transfer });
...
pub const CacheLayer = struct {
    ...
    fn _request(ptr: *anyopaque, ctx: Context, req: Request) anyerror!void {
        ...
        if (network.cache != null and req.method == .GET) {
            const arena = try network.app.arena_pool.acquire(.{ .debug = "CacheLayer.request" });
...
// RobotsContext is no longer parameterised — RobotsLayer is a concrete type now
...
pub const WebBotAuthLayer = struct {
    ...
    fn _request(ptr: *anyopaque, ctx: Context, req: Request) anyerror!void {
        ...
        if (self.auth) |auth| {
            ...
            // sign into req.headers before forwarding
...
pub fn LayerStack(comptime layer_types: anytype) type {
    return struct {
        // raw heap pointers, one per layer type
        ptrs: [layer_types.len]*anyopaque,
        // wired Layer handles, built once at init
        ...
        pub fn init(allocator: Allocator, transport: *Transport, instances: anytype) !Self {
            ...
            // wire inner: each layer's inner points to the next layer
...
pub fn init(allocator: Allocator, network: *Network) !*Client {
    const transport = try Transport.init(allocator, network);
...
pub fn request(self: *Client, req: Request) !void {
    const arena = try self.transport.network.app.arena_pool.acquire(.{ .debug = "Client.request.ctx.arena" });
    defer self.transport.network.app.arena_pool.release(arena);

    const ctx = Context{ .network = self.transport.network, .arena = arena };
...
pub const Transfer = struct {
    ...
    fn headerDoneCallback(transfer: *Transfer, conn: *const http.Connection) !bool {
        ...
        if (transfer.max_response_size) |max_size| {
            if (transfer.getContentLength()) |cl| {
                if (cl > max_size) return error.ResponseTooLarge;
            }
        }

        transfer.req.notification.dispatch(.http_response_header_done, &.{ .transfer = transfer });
